...
def gluballs(graph_view, Retry):
    ...
    for vertex in graph_view.vertices:
        ...
        if len(vertices) > 1:
            ...
            summary.tag("gluball")
            summary.gluball_nvertices = sum(getattr(x, "gluball_nvertices", 1)
                                            for x in vertices)
            raise Retry
...
def categorize(graph_view):
    ...
    for vertex in graph_view.vertices:
        ...
        for particle in final_state_particles:
            by_pdgid.setdefault(particle.pdgid, []).append(particle)
        for _, particles in by_pdgid.iteritems():
            if len(particles) > 1:
                graph_view.summarize_vertices(p.end_vertex \
                        for p in particles).tag("category")
...
def chainmail(graph_view, Retry):
    ...
    for particle in graph_view.particles:
        ...
        if len(siblings) > 1:
            ...
            summary.multiple_count = sum(getattr(x, "multiple_count", 1) \
                    for x in siblings)
            raise Retry

_jet_algos = ("kt", "cambridge", "antikt", "genkt", "cambridge_for_passive",
              "genkt_for_passive", "ee_kt", "ee_genkt")
class Jets(Transform):
    _name = "Jets"
    _args = [Arg("algorithm", str, "Jet Algorithm", default="antikt",
                 choices=_jet_algos),
             Arg("r", float, "Delta R for the Jet Algorithm", default=0.4),
             Arg("n_max", int, "Maximum number of jets to form", default=5),
             Arg("tracks", Arg.bool, "Only cluster charged particles",
                 default=False)]
    def __call__(self, graph_view):
        ...
        def pselect(particle):
            return particle.final_state and not particle.invisible \
                    and (particle.charge != 0 if track_jets else True)
        ...
        for i, jet in enumerate(sorted(jets, key=pt, reverse=True)):
            print "Created jet: np=%2i, %r, %r" \
                    % (len(jet.particles), jet.p, jet.e)
...
def contract_clusters(graph_view):
    ...
    for vertex in graph_view.vertices:
        ...
        cluster_ends = set(p.end_vertex for p in Walk.particles
                           if p.final_state)
...
@Transform.decorate("Pluck", \
        args=[Arg("start", float, "start of range to keep", default=6),
              Arg("end", float, "end of range", default=0),
              Arg("param", str, "parameter of interest", default="pdgid"),
              Arg("keep_down", int, "max depth to descend from vertex",
                  default=8),
              Arg("keep_up", int, "max depth to ascend from vertex",
                  default=20)])
def pluck(graph_view, start, end, param, keep_down, keep_up):
    ...
    for particle in keep_particles:
        max_depth = keep_down
        graph_view.walk(particle.start_vertex, vertex_action=walker,
                        particle_action=walker)
        max_depth = keep_up
        graph_view.walk(particle.start_vertex, vertex_action=walker,
                        particle_action=walker, ascend=True)
...
class Cut(Transform):
    ...
    _args = [Arg("cut", float, "cut value", default=5),
             ...
             Arg("mothers", Arg.bool, "cut on mother parameters",
                 default=False),
             Arg("daughters", Arg.bool, "cut on daughter parameters",
                 default=False),
             Arg("final_state", Arg.bool, "cut on final state particles only",
                 default=True),
            ]
    ...
    def __call__(self, graph_view):
        ...
        for particle in particles:
            ...
            for particle in loop_over:
                if not cutter(particle):
                    particles_to_walk.add(particle)
                    # also display the particle whose mother or daughter
                    # passed the cut
                    if mothers or daughters:
                        particles_to_walk.add(particle)

        for particle in particles_to_walk:
            graph_view.walk(particle, vertex_action=mark,
                            particle_action=mark, ascend=True)
        ...
        for vertex in graph_view.vertices:
            if passed_tag in vertex.tags:
                cut_daughters = [p for p in vertex.outgoing
                                 if passed_tag not in p.tags]
        ...
        for particle in graph_view.particles:
            ...
            if particle.start_vertex in keep:
                # Don't discard
                continue
