...
def qcd_rgb(layout):
    ...
    cmap = dict(zip(unique_colors, ["red", "lime", "blue"] * \
            (len(unique_colors)//3+1)))
    amap = dict(zip(unique_colors, ["cyan", "magenta", "yellow"] * \
            (len(unique_colors)//3+1)))
...
def qcd_rainbow(layout):
    ...
    cmap = dict([(unique_colors[i], rainbow_color(color_values[i], 0.5))\
            for i in range(len(unique_colors))])
    amap = dict([(unique_colors[i], rainbow_color(color_values[i], 0.8))\
            for i in range(len(unique_colors))])
    set_color(layout, cmap, amap)
