...
def print_type_help(cls, links=False):
    ...
    if cls._merge_classes:
        helptext2 = "All tools of the type '{0:s}' are merged into one class. "\
                "There are 'base' classes (one of which must be included) "\
                "and supplementary classes.".format(name)
    else:
        helptext2 = "Tools of type '{0}' are simple tools, and can be applied "\
        "several times (if sensible)".format(name)
    text.append(str(helptext2))

    # The following is an automatic example generator.
    # It avoids using "None" and "" arguments if it can help it
    def nonnull(args):
        return [a for a in args if not a[1].default in [None, ""]]

    documented_classes = [v for v in tool_classes[name].values() \
            if v.__doc__ is None or not "UNDOCUMENTED" in v.__doc__]
    sortable_classes = ((len(nonnull(t.args())), len(t.args()), t) \
            for t in reversed(documented_classes))
    ...
    if len(arglist) == 0:
        ...
        example = "--{0} {1}:{2} or --{3} {4}:{5}={6}"\
            .format(cls._type, tool._name, argcls.default, cls._type, \
                    tool._name, argname, argcls.default)
        text.append("This is a list of all {0}s with their arguments. "\
                "Arguments are separated by colons ':'.".format(name))
        text.append("Arguments can be positional or specified by name.")
        text.append("")
        text.append("Example: {0}".format(example))

    else:
        text.append("Here is a list of all {0}s with their arguments. "\
                "Arguments are separated by colons ':'.".format(name))
        (_, arg1cls), (arg2name, arg2cls) = arglist[:2]
        example = "--{0} {1}:{2}:{3}={4}".format(cls._type, tool._name, \
                arg1cls.default, arg2name, arg2cls.default)
        text.append("Arguments can be positional or specified by name.")
        text.append("")
        text.append("Example: {0}".format(example))
    ...
    for _, tool in sorted(tool_classes[name].iteritems()):
...
def print_tool_help(indent, tool, links=False):
    ...
    if links:
        helptext += "\n![example image]({0}_{1}.png)"\
                .format(tool._type.lower(), tool._name.lower())

    from inspect import getfile

    root_dir = dirname(pkg_resources.resource_filename("mcviz", "main"))
    definition = getfile(tool)[len(root_dir):].rstrip("co")
    ...
    if tool.args():
        for arg in tool.args():
            ...
            if arg_obj.choices is None:
                ...
                choices = " (choices: {0})".format(", ".join("'{0}'".format(c)\
                        for c in arg_obj.choices))
            text.append("{0} * {1}='{2}': {3}{4}".format(indent, arg_name, \
                        arg_obj.default, arg_obj.doc, choices))
...
def run_help(parser, args):
    ...
    tool_list = reduce(list.__add__, (tc.values()\
                for tc in tool_classes.values()))
    tool_types_lower = dict((tn.lower(), ty)\
            for tn, ty in tool_types.iteritems())

    tool_map = {}
    for tool in tool_list:
        tool_map.setdefault(tool._name.lower(), []).append(tool)
...
#        print("For a list of examples, type %(prog)s --help examples")
    ...
    if topic == "examples":
        ...
        print("Unknown help topic '{0}'. Known topics: {1} "\
                "(and all tool names)".format(topic, HELP_TOPICS))
