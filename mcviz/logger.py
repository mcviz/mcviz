...
def get_log_handler(singleton={}):
    ...
    template = "[$BOLD%(name)-23s$RESET][%(levelname)-18s]  %(message)s"
    if os.isatty(handler.stream.fileno()) or FORCE_COLOR:
        handler.setFormatter(ColoredFormatter(insert_seqs(template)))
    else:
        handler.setFormatter(MCVizFormatter(remove_seqs(template)))
