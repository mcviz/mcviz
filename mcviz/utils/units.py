...
class Units(object):
    ...
    def set_energy(self, string):
        ...
        if not self.energy_mag_string:
            ...
            if string.lower() == "gev":
                self.energy_mag = 1
            elif string.lower() == "mev":
                self.energy_mag = 0.001
            elif string.lower() == "kev":
                self.energy_mag = 0.000001
            elif string.lower() == "tev":
                self.energy_mag = 1000
            else: raise RuntimeError("unit {0} not recognised".format(string))
            LOG.debug("setting energy unit to {0} factor {1:g}"\
                    .format(string, self.energy_mag))
        else:
            LOG.error("unit is already {0:s}, not overriding with {1:s}"\
                    .format(self.energy_mag_string, string))
    ...
    def set_length(self, string):
        ...
        if not self.length_mag_string:
            ...
            if string.lower() == "mm":
                self.length_mag = 1
            elif string.lower() == "cm":
                self.length_mag = 10
            else: raise RuntimeError("unit {0} not recognised".format(string))
            LOG.debug("setting length unit to {0:s}".format(string))
        else:
            LOG.error("unit is already {0:s}, not overriding with {1:s}"\
                    .format(self.length_mag_string, string))
    ...
    def pick_energy_mag(self, value):
        """
        Take an energy in the input units, return it in an appropriates
        display unit
        """
    ...
    def initial_check(self, initial_particle):
        ...
        If units are automatic, the best units will be picked,
        else a warning is issued.
        """
        energy = self.energy_mag * initial_particle.e
        LOG.verbose("initial particle of energy {0:.4g}{1:s}eV"
                    .format(*self.pick_mag(energy)))
        if energy > 100000:
            new = self.pick_mag(energy*0.0000001)
        elif energy < 100:
            new = self.pick_mag(energy*100)
        else:
            LOG.verbose("initial particle of energy {0:.4g}{1:s}eV"
                        .format(*self.pick_mag(energy)))
            return
        ...
        if self.auto:
            ...
            LOG.info(info.format(*self.pick_mag(initial_particle.e * \
                    self.energy_mag), new=new[1]))
        else:
            warning = "Input has beam particle with an energy of "\
                      "{0:.4g}{1:s}eV, consider setting "\
                      "--units={new:s}eV or auto if this is incorrect"
            LOG.warn(warning.format(*self.pick_mag(initial_particle.e * \
                    self.energy_mag), new=new[1]))
        return

    def energy_formatter(self, value):
        """Return value as a string with energy units"""
        if not value:
            return ""
        return "{0:.4g}{1:s}eV".format(*self.pick_mag(value))

def main():
    """Trial a few values to check formatting"""
    unit = Units()
    def trial(value):
        print("{0:f} GeV is represented: ".format(value)
              + unit.energy_formatter(value))
