...
def variable_item(record):
    ...
    number, record = int(float(record[0])), record[1:]
    return items(number, record)
...
def make_record(record):
    ...
    if type_ == "E":
        ...
        units = Units(record[0] + " " + record[1])
        return units

    elif type_ == "F":
        return HPDF._make(record)

def load_single_event(event_lines, args):
    """
    ...
    for record in map(make_record, event_lines):
        if isinstance(record, Units):
            ...
            vertex_incoming.setdefault(int(record.vertex_out_barcode), set())\
                    .add(particle)
    ...
    for vertex_barcode in sorted(vertex_incoming.keys()):
        ...
        if not vertex_barcode:
            ...
            for particle in sorted(incoming_particles):
                vertices[vno] = Vertex(vno, [particle])
                vno -= 1
        else:
            vertex = vertices[vertex_barcode]
            vertex.incoming = incoming_particles
    ...
    if len(initial_particles) != 2:
        ...
        for particle in initial_particles: LOG.debug(repr(particle))

    # Construct "initial" vertices
    for initial_particle in initial_particles:
        if str(initial_particle.no) in \
                (event.beam_p1_barcode, event.beam_p2_barcode):
            pass
