...
def make_pythia_graph(records):
    """De-serialise event into particle and vertex maps"""
    ...
    for particle in particles:
        ...
        if frozenset(particle.mothers) in vertex_dict:
            ...
            for vertex in vertex_dict.itervalues():
                for mother in particle.mothers:
                    if mother in vertex.incoming:
                        found_v = vertex
                        #print >> stderr, map(lambda x: x.no, vertex.incoming),
    ...
    for particle in particles:
        if particle.final_state:
            ...
            if particle.color or particle.anticolor:
                ...
                LOG.debug("final state particle: {0:s} color: {1:d} anticolor "
                          "{2:d}".format(repr(particle), particle.color,
                                         particle.anticolor))
...
def load_event(args):
    ...
    with open(filename) as handle:
        lines = try_decompress(handle.read()).split("\n")
    ...
    def maybe_num(value):
        """Make a float from a string... maybe"""
        try:
            return float(value)
        except ValueError:
            return value
